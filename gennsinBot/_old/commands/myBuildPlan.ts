import { ChatInputCommandInteraction, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ComponentType, StringSelectMenuInteraction, InteractionReplyOptions, InteractionEditReplyOptions } from 'discord.js';
import { calculateBuildCost, calculateTalentCost, calculateFullBuildCost } from '../utils/buildCalculator';
import { getUserCharacters } from '../utils/userData';
import { findEnglishCharacterName } from '../utils/characterMapping';
import { OPTION_NAMES } from '../constants/commands';

// モラをフォーマットする関数
function formatMora(mora: number): string {
...
function findCharacterIdByName(characterName: string, userCharacters: any): string | null {
    for (const [characterId, savedCharacter] of Object.entries(userCharacters)) {
        const saved = savedCharacter as any;
        if (saved.characterName.includes(characterName) || 
            characterName.includes(saved.characterName)) {
            return characterId;
        }
    }
    return null;
...
// 育成コスト詳細を表示
async function showBuildCostDetails(interaction: ChatInputCommandInteraction | StringSelectMenuInteraction, savedCharacter: any, targetLevel: number): Promise<void> {
...
// 育成プランオプションを表示
async function showBuildPlanOptions(interaction: ChatInputCommandInteraction | StringSelectMenuInteraction, savedCharacter: any, currentLevel: number): Promise<void> {
    // 英語名を取得
    const englishName = findEnglishCharacterName(savedCharacter.characterName);
    if (!englishName) {
...
// キャラクター選択メニューを表示
async function showCharacterSelectMenu(interaction: ChatInputCommandInteraction, userCharacters: any, targetLevel?: number): Promise<void> {
    ...
    // セレクトメニューのオプションを作成
    const options = characters.slice(0, 25).map(([characterId, character]: [string, any]) => {
...
// 保存されたキャラクターの育成コスト計算
export async function handleMyCharacterBuild(interaction: ChatInputCommandInteraction): Promise<void> {
    try {
...
// 保存されたキャラクターの天賦育成コスト
export async function handleMyCharacterTalent(interaction: ChatInputCommandInteraction): Promise<void> {
    try {
        const characterName = interaction.options.getString(OPTION_NAMES.CHARACTER_NAME)!;
        const talentType = interaction.options.getString(OPTION_NAMES.TALENT_TYPE) as 'normal' | 'skill' | 'burst';
        const targetLevel = interaction.options.getInteger(OPTION_NAMES.TARGET_LEVEL)!;
        const userId = interaction.user.id;
