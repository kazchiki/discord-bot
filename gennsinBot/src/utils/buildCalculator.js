...
// 必要昇格段階を計算
function getRequiredAscensions(currentLevel, targetLevel) {
    const requiredAscensions = [];
    
    for (let ascension = 1; ascension <= 6; ascension++) {
        const ascensionData = ASCENSION_LEVELS[ascension];
...
// 経験値を経験書に変換
function convertExpToBooks(totalExp) {
...
// レベル上げ必要経験値を計算
function calculateExpRequirement(currentLevel, targetLevel) {
    let totalExp = 0;
    ...
    for (const stage of stages) {
        ...
        if (stageStart < stageEnd) {
            const stageExp = EXP_REQUIREMENTS[stage.range];
...
// キャラクター育成コスト計算
async function calculateBuildCost(characterName, currentLevel, targetLevel) {
    try {
        // 日本語結果を取得
        ...
        for (const ascension of requiredAscensions) {
            const ascensionKey = `ascend${ascension}`;
...
// 天賦育成コスト計算
async function calculateTalentCost(characterName, talentType, currentLevel, targetLevel) {
    try {
        genshindb.setOptions({ resultLanguage: 'Japanese' });
...
async function calculateFullBuildCost(characterName) {
    try {
